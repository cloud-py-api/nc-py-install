...
def test_subpackages(package_name, version):
    ...
    r = subprocess.run(
        ARGUMENTS + [package_name] + ["--check"], capture_output=False
    )
    assert r.returncode == 1
    install_string = package_name + "==" + version if version else package_name
    # Install must succeed
    r = subprocess.run(
        ARGUMENTS + [install_string] + ["--install"], capture_output=False
    )
    assert r.returncode == 0
    if version:
        r = subprocess.run(
            ARGUMENTS + [package_name] + ["--update"], capture_output=False
        )
        assert r.returncode == 0
    # Clean up, by deleting package.
    r = subprocess.run(
        ARGUMENTS + [install_string] + ["--delete"], capture_output=False
    )
    assert r.returncode == 0
